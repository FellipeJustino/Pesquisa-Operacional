...
public class Vogel {
    ...
    public static int calcularPenalidade(int i, int qtdConsumidores, int[][] custos, boolean[] colunaSatisfeita) {
        int menor1 = 999999;
        int menor2 = 999999;
    ...
    public static int[][] vogel(int qtdFornecedores, int qtdConsumidores, int[] capacidade, int[] demanda, int[][] custos) {
        ...
        while (linhasAtivas > 0 && colunasAtivas > 0) {
            ...
            for (int i = 0; i < qtdFornecedores; ++i) {
                if (linhaSatisfeita[i]) {
                    continue;
                }

                int penalidade = calcularPenalidade(i, qtdConsumidores, custos, colunaSatisfeita);
            ...
            for (int j = 0; j < qtdConsumidores; ++j) {
                if (colunaSatisfeita[j]) {
                    continue;
                }

                int pen = calcularPenalidade(j, qtdFornecedores, custos, linhaSatisfeita);
